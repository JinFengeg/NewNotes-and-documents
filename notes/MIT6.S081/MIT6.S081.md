...
exec系统调用移除了调用进程的内存，用一个在文件系统中保存到文件中的内存镜像加载。文件必须是一个特殊格式，指定文件保存指令的部分。哪个部分是数据，哪个部分是开始等等。xv6使用ELF格式，当exec成功执行后，并不会返回到调用进程。代替的，指令从文件开始执行的地方加载，入口点被声明在ELF头部。exec携带两个参数：文件包含的可执行的名称，以及一个字符数组参数。例如：
