...
#define ICM20608_CHAN(_type, _channel2, _index)                    \
	{                                                             \
		.type = _type,                                        \
		.modified = 1, /*类型相同，但意义不同，比如不同轴的偏转*/                                       \
		.channel2 = _channel2,                                \
...
static const struct iio_chan_spec icm20608_channels[] = {
	/* 温度通道 */
	{
		...
		.scan_index = INV_ICM20608_SCAN_TEMP,/*类型的索引*/
		.scan_type = {/**/
				.sign = 's',
				.realbits = 16,
				.storagebits = 16,
			   ...
			   int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:								/* 读取ICM20608加速度计、陀螺仪、温度传感器原始值 */
		iio_device_claim_direct_mode(indio_dev);		/* 保持direct模式 ，就是
		从现在起该iio设备的mode保持成direct模式了	*/
		...
		return ret;
	case IIO_CHAN_INFO_SCALE:/*由访问的通道的目录项决定*/
		switch (chan->type) {
		case IIO_ANGL_VEL:
			mutex_lock(&dev->lock);
...
{
	int ret = 0;
	struct iio_poll_func *pf = p;/*其实就是申请者结构体*/
	struct iio_dev *indio_dev = pf->indio_dev;
	struct icm20608_dev *dev = iio_priv(indio_dev);
	u8 data[ICM20608_OUTPUT_DATA_SIZE];
	int int_status = 0;
//printk("enter trigger_handler\n");
	mutex_lock(&dev->lock);
...
static int icm20608_probe(struct spi_device *spi)
{
	...
	/*  1、申请iio_dev内存 */
	/*上来先将自定义结构体存入iio_dev中，spi中是kzmalloc分配
	这里是分配dev指针，并将指针赋值给iio设备中*/
	...
	/* 4、iio_dev的其他成员变量 */
	indio_dev->dev.parent = &spi->dev;/*在spi节点下看到iio设备*/
	indio_dev->info = &icm20608_info;/*这是iio设备的操作结构体*/
	indio_dev->name = ICM20608_NAME;/*iio设备名称*/	
	...
	/* 5、触发缓冲区设置 */
	/*说是缓冲区设置，但实际上是
	设置iio_buffer，创建iio_pollfunc并*/
	ret = devm_iio_triggered_buffer_setup(&spi->dev, indio_dev,
						 &iio_pollfunc_store_time,/*这两个
						 中断上下部就是pollfunc向触发器申请虚拟中断的上下部*/
						 &icm20608_trigger_handler,
						 NULL);

	/* 6、申请trigger,并初始化 */
	/*因为申请的是trigger，不再和设备树相关，这里关联的是
	应该还是和设备树的设备结构体有关，此API可能是对
	trig->dev成员进行赋值了，*/
...
/*自定义的结构体中没有parent成员，这是触发器的成员*/
	...
	/* 8、ICM20608中断初始化 */
	/*spi是设备树生成的节点，spi子系统已经将子设备
	节点中的中断获取到irq了*/
	ret = devm_request_irq(&indio_dev->dev, spi->irq,
			       &iio_trigger_generic_data_rdy_poll,/*
				   中断服务函数中只需要执行iio_trigger_poll函数
				   从而触发虚拟中断即可*/
			       IRQF_TRIGGER_HIGH,
			       "inv_mpu",
			       dev->trig);
	...
	/* 9、注册iio_dev */
	/*最后才是注册iio设备，毕竟此时iio缓冲区/
	iio设备/iio触发器都已经注册成功了*/
