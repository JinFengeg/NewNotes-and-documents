...
SENSOR_INT_DATA_GET(ret, voltage3_raw, str, in_voltage3_raw);
 ...
 fd_w = open("./test/ADC_file",O_CREAT|O_RDWR);
if(-1 == fd_w)
{
	perror("open error");
	exit(-1);
...
ftruncate(fd_w,0);
 ...
 if(ret == 0) {
   raw[0] = ad7606.in_voltage0_raw;
   raw[1] = ad7606.in_voltage1_raw;
   raw[2] = ad7606.in_voltage2_raw;
   raw[3] = ad7606.in_voltage3_raw;
   raw[4] = ad7606.in_voltage4_raw;
   raw[5] = ad7606.in_voltage5_raw;
   raw[6] = ad7606.in_voltage6_raw;
   raw[7] = ad7606.in_voltage7_raw;
 printf("\r\n 原始值:\r\n");
...
printf("in_voltage6_act = %f\n",ad7606.in_voltage6_act);
 ...
 printf("in_voltage_scale =%f\n",ad7606.in_voltage_scale);
	...
	if(sizeof(raw) != write(fd_w,raw,sizeof(raw)))
	{
		perror("adc write error\n");
		exit(-1);
	}
 }else{
   printf("read error\r\n");
