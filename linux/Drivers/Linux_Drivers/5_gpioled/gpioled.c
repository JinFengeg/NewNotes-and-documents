...
int led_gpio; /*led所使用的GPIO编号*/
    ...
    /*gpio子系统的api是根据gpio编号实现对gpio操作的
    当无法获得gpio编号时，使用gpiod子系统，这种
    子系统是通过对gpio_dest结构体操作实现的*/
...
return 0;
    ...
    static int __init led_init(void)
    {
        ...
        /*1.获取设备节点：gpioled*/
        gpioled.nd = of_find_node_by_path("/gpioled");/*这些节点三
        在根节点下的，对于非跟节点下的都是&追加节点*/
        if(gpioled.nd == NULL)
        {
        ...
        free_gpio:
        gpio_free(gpioled.led_gpio);/*释放申请的gpio编号*/
        return -EIO;
    }
