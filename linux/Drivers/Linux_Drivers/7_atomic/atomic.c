...
atomic_t lock; /* 原子变量 */
    ...
    {
        /*通过判断原子变量的值来检查LED有没有被其他的应用使用*/
        if(!atomic_dec_and_test(&gpioled.lock))
        {
            atomic_inc(&gpioled.lock);//小于0的话就加1使其原子变量等于0
            return -EBUSY;//led被使用，返回忙
        }//当只被使用一次的话跳过if，被使用多次的话本次不打开文件。
...
return 0;
    ...
    {/*
    atomic_t是结构体类型，里面只有一个参数就算volatile int，
    之所以特意定义原子类型，为了只使用API实现操作*/
        int ret =0;
        const char *str ;
        //但其实上面是在APP操作的时候检验的，在挂载驱动模块的时候应该对其进行初始化，因为挂载只有一次，而操作文件可能有很多次
        gpioled.lock = (atomic_t)ATOMIC_INIT(0);
        //其实在上面就可以初始化了，为啥还要多一行。
        atomic_set(&gpioled.lock,1);//到此就结束了。
        /*设置LED所使用的GPIO*/
