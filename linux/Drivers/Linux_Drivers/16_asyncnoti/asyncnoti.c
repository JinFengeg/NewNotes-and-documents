...
#include <linux/fcntl.h>
 ...
 struct key_dev{
    ...
    int irq_num;//中断号
    //去掉了自选锁
    atomic_t status; //按键状态，这里改成了原子操作。
...
wake_up_interruptible(&key.r_wait);//唤醒读等待队列头
if(key.async_queue)//判断下是否初始化了这个指针
kill_fasync(&key.async_queue, SIGIO, POLL_IN);
 }else if(1 == current_val &&!last_val){

atomic_set(&key.status,KEY_RELEASE);
wake_up_interruptible(&key.r_wait);//唤醒读等待队列头，这里的等待队列项还没有定义。
if(key.async_queue)
kill_fasync(&key.async_queue, SIGIO, POLL_IN);//当按下和谈起的时候都向应用程序发送给信号，让信号处理函数读取
}else{

atomic_set(&key.status,KEY_KEEP);//这里把自旋锁的保护区换成了无法被打断的API来操作数据，没啥区别
}
...
//上面这个API是只有当按键谈起或在按下的时候才能唤醒应用进程。哪个进程调用驱动就唤醒哪个进程。如果没有按下或谈起就休眠进程
if(ret)
return ret;
}//刚开始进程是不被阻塞的，一直在read，驱动一直在回应，只有不满足上面函数的时候才会阻塞掉进程。
    //之前是把状态读取出来，这回就是把状态信息发送给应用程序
    ret =copy_to_user(buf,&key.status,sizeof(int));
    //状态重置

    atomic_set(&key.status, KEY_KEEP);//原子操作就是对数据进行操作。
    return ret;
 }
 ...
 {
    unsigned int mask =0;
    poll_wait(filp,&key.r_wait,wait);//通过这里将进程阻塞
    if(KEY_KEEP!=atomic_read(&key.status))
